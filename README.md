...
- **文生动画 (Text-to-Animation)**（纯代码矢量动效、0 图像 Token 消耗、8 大视觉组件、FFmpeg 管道流式压制、多主题支持）
...
### 文生动画 (Text-to-Animation) ⭐ 推荐

无需调用耗时昂贵的生图大模型，纯代码动态矢量逐帧压制：

- **0 图像 Token 消耗**：纯本地矢量绘制，完全无外部生图 API 费用。
- **8 大科技视觉版式**：
  1. `cards_grid`：亮点矩阵卡片
  2. `comparison`：左右红绿痛点优势对比
  3. `data_chart`：动态金融行情走势与折线图
  4. `steps_flow`：向导式操作步骤流 (Step 1 ➔ Step 2 ➔ Step 3)
  5. `metrics_grid`：核心数据指标大数字仪表盘
  6. `code_terminal`：极客终端打字机与代码高亮
  7. `quote_focus`：震撼观点名言金句发光聚焦
  8. `call_to_action`：尾声行动号召与 GitHub 开源引导
- **5 种精选视觉色彩主题**：科技赛博（默认）、商务典雅蓝、黑金尊贵、极光霓虹紫、极客黑客绿。
- **1080P/720P 横竖屏自适应**：支持 16:9 横屏 (1920x1080) 与 9:16 手机短视频 (1080x1920)。
- **一键极速换配音**：成片后无需重新生图，支持一键切换 MiMo 音色并重新毫秒对齐混流。
- **可视化分镜剧本编辑**：支持在线批量微调各分镜标题、副标题、标签、版式类型与解说台词。
...
.
├── app.py
├── animation_engine.py      # 动态矢量动画逐帧渲染器与 FFmpeg 管道
├── animation_mvp.py         # 文生动画调度器、LLM分镜规划与 API 路由
├── video_mvp.py             # 文生漫画视频调度器与生图流程
├── requirements.txt
├── .env.example
├── Dockerfile
├── docker-compose.yml
